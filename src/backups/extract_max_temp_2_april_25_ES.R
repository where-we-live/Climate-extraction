...
nsat <- nc_open("http://thredds.northwestknowledge.net:8080/thredds/dodsC/agg_met_tmmx_1979_CurrentYear_CONUS.nc#fillmismatch")
...
temp_data <- ncvar_get(nsat, "daily_maximum_temperature", 
...
for (i in seq_along(lon_ind)) {
  for (j in seq_along(lat_ind)) {
    temp_df <- data.frame(
      ...
      lat = lat[lat_ind[j]], 
      date = dates,
      daily_maximum_temperature = temp_data[i, j, ]
    )
